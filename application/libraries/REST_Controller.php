...
abstract class REST_Controller extends MX_Controller {
            ...
            elseif ($data !== null) {
                ...
                if (method_exists($this->format, 'to_' . $this->response->format)) {
                    ...
                    if ($this->response->format === 'array') {
                        $output = $this->format->factory($output)
                                               ->{'to_json'}();
                    }
                } else {
                    // If an array or object, then parse as a json, so as to be a 'string'
                    if (is_array($data) || is_object($data)) {
                        $data = $this->format->factory($data)
                                             ->{'to_json'}();
                    }
    ...
    protected function _detect_input_format() {
        ...
        if (empty($content_type) === false) {
            ...
            $content_type = (strpos($content_type, ';') !== false ? current(explode(';', $content_type)) :
                $content_type);
    ...
    protected function _detect_method() {
        ...
        return in_array($method, $this->allowed_http_methods) && method_exists($this, '_parse_' . $method) ? $method :
            'get';
    }
    ...
    protected function _detect_api_key() {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] :
            ...
            if (!($row = $this->rest->db->where($this->config->item('rest_key_column'), $key)
    ...
    protected function _log_request($authorized = false) {
        ...
        $is_inserted = $this->rest->db->insert($this->config->item('rest_logs_table'), [
                ...
                'params'     => $this->_args ?
                    ($this->config->item('rest_logs_json_params') === true ? json_encode($this->_args) :
                        serialize($this->_args)) : null,
    ...
    protected function _check_limit($controller_method) {
        ...
        $time_limit = (isset($this->methods[$controller_method]['time']) ? $this->methods[$controller_method]['time'] :
            3600); // 3600 = 60 * 60

        // Get data about a keys' usage and limit to one row
        $result = $this->rest->db->where('uri', $limited_uri)
        ...
        elseif ($result->hour_started < (time() - $time_limit)) {
            ...
            $this->rest->db->where('uri', $limited_uri)
                           ->where('api_key', $api_key)
    ...
    protected function _log_access_time() {
        ...
        return $this->rest->db->update($this->config->item('rest_logs_table'), $payload, [
            'id' => $this->_insert_id
        ]);
    }
    ...
    protected function _check_access() {
        ...
        $accessRow = $this->rest->db->where('key', $this->rest->key)
        ...
        return $this->rest->db->where('key', $this->rest->key)
