...
class Mitem extends CI_Model {
	...
	function giftcode($id,$code) {
		...
		if (count($codeinfo)==0)
			return array('err'=>3,'errmsg'=>'Giftcode không đúng');
		$date = date("Ymd");
		if ($date > $codeinfo['exdate'])
			return array('err'=>3,'errmsg'=>'Giftcode đã hết hạn');
		if ($codeinfo['status']==0)
			return array('err'=>3,'errmsg'=>'Giftcode chưa kích hoạt');
		if ($codeinfo['status']==2)
			return array('err'=>3,'errmsg'=>'Giftcode sử dụng rồi');
		$result = array();
		switch ($codeinfo['codetype']) {
			case 1:
				$status = $this->giftcode_use_type1($id,$codeinfo);
				if ($status==0)
					$result = array('err'=>3,'errmsg'=>'Vui lòng thử lại vào ngày mai');
				else
					$result = array('err'=>0,'errmsg'=>'Kích hoạt thành công, may mắn +5%');
				break;
			case 2:
				...
				if ($status==0)
					...
					$result = array('err'=>0,'errmsg'=>"Kích hoạt thành công\n+$status Thiên Sứ");
				break;
			default:
				return array('err'=>3,'errmsg'=>'Lỗi hệ thống, vui lòng báo hỗ trợ');
		}
		return $result;
	}
